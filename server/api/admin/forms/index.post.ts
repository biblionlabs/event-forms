export default defineEventHandler(async (event) => {
  ...
  await db.prepare(
    `INSERT INTO forms (id, name, description, thank_you_title, thank_you_message, thank_you_image_url, identifier_fields, cookie_fields, session_required_fields)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    id,
    body.name,
      ...
      if (step.fields && Array.isArray(step.fields)) {
        for (const field of step.fields) {
          await db.prepare(
            ...
            generateId(), stepId, id, field.field_key, field.label,
